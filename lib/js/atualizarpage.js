...
// Pesquisar Histórico por número de intervalo e observação
document.addEventListener("input", (e) => {
    const vDataSearch = e.target.dataset.search 
    if(vDataSearch) {
        const vTermsSearch = e.target;
        const vUpdateList = vTermsSearch.dataset.update;

        const vForm = new FormData();
        vForm.append(vTermsSearch.name, vTermsSearch.value)
        ...
        .then(data => {
            const vList = document.getElementById(vUpdateList);
